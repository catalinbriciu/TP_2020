...
int Valid(int k)
{
	for (i = 1; i < k; i++) // verificã dacã elementul din varf este diferit de elementele precedente din stiva//
		if (v[i] == v[k])
			return 0;
		else
			return 1;
	...
	while (k > 0)  // cât timp stiva nu e vidã//
		{
			isS = 0;
			isV = 0;
			if (k <= n) // nu face sens depãºirea nivelului n în stivã//
				do { // repetã cât timp//
					isS = Succesor(k);
					if (isS) isV = Valid(k);
